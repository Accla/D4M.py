...
def test_sorted_union(test1, test2, returnbool, exp, returnexp1, returnexp2):
    if returnbool:
        union, index_map_1, index_map_2 = D4M.util.sorted_union(test1, test2, return_index=True)
...
def test_sorted_intersect(test1, test2, returnbool, exp, returnexp1, returnexp2):
    if returnbool:
        intersection, index_map_1, index_map_2 = D4M.util.sorted_intersect(test1, test2, return_index=True)
...
@pytest.mark.parametrize("test_word,suffix,exp_word",
                         [('1.0', '.0', '1'),
                          ('1.1', '.0', '1.1'),
                          ('abcd', 'c', 'abcd'),
                          ('abcd', 'd', 'abc')
...
@pytest.mark.parametrize("test_object,can_san",
                         [([0, 1, 2], True),
                          (0, True),
                          ('ab', True),
                          ({0, 1, 2}, False),
...
def test_sanitize(test, convert, prevent_upcasting, exp):
    assert np.array_equal(exp, D4M.util.sanitize(test, convert=convert, prevent_upcasting=prevent_upcasting))
...
@pytest.mark.parametrize("row,col,val,func,agg_row,agg_col,agg_val",
                         [(['a', 'a', 'b'], ['A', 'A', 'B'], [1, 2, 3], D4M.util.add, ['a', 'b'], ['A', 'B'], [3, 3]),
                          (['a', 'a', 'b'], ['A', 'A', 'B'], [1, 2, 3], D4M.util.first,
                           ['a', 'b'], ['A', 'B'], [1, 3]),
                          (['a', 'a', 'b'], ['A', 'A', 'B'], [1, 2, 3], D4M.util.last, ['a', 'b'], ['A', 'B'], [2, 3]),
                          (['a', 'a', 'b'], ['A', 'A', 'B'], [2, 2, 3],
                           D4M.util.times, ['a', 'b'], ['A', 'B'], [4, 3]),
...
@pytest.mark.parametrize("new_entry,sorted_array,array_of_indices,exp_sorted_array,exp_array_of_indices",
                         [('aaB', np.array(['aA', 'aB', 'bB']), np.array([0, 1, 1, 0, 2]),
                           np.array(['aA', 'aB', 'aaB', 'bB']), np.array([0, 1, 1, 0, 3, 2])),
                          ('c', np.array(['aA', 'aB', 'bB']), np.array([0, 1, 1, 0, 2]),
                           np.array(['aA', 'aB', 'bB', 'c']), np.array([0, 1, 1, 0, 2, 3]))
                          ])
def test_sorted_append(new_entry, sorted_array, array_of_indices, exp_sorted_array, exp_array_of_indices):
    new_sorted_array, new_array_of_indices = D4M.util.sorted_append(new_entry, sorted_array, array_of_indices)
...
def test_update_indices(array_of_indices, sorted_bad_indices, size, offset, mark, exp_array):
    offset, mark = replace_default_args(D4M.util.update_indices, offset=offset, mark=mark)
    updated_array = D4M.util.update_indices(array_of_indices, sorted_bad_indices, size, offset=offset, mark=mark)
